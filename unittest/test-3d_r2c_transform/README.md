...
is chosen to be the $z$ axis, and FFTW3 splits this axis among all MPI
processes. 
...
To run the executable, you need to specify tolerance in the
argument list. Optionally, by also including an additional
argument you can output the data to csv
(text) files with the names ``output_%.txt'' where ``%'' is
replaced by the processor number. The python script ``plot.py''
can be used to visualise the output.
