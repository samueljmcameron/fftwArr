...
int main(int argc, char **argv)
{
  ...
  try {
    numcalls = std::stod(argv[2]);
  } catch (const std::logic_error &e) {
    throw std::runtime_error("Need to input number of times to try the calculation"
			     + std::string(", e.g. ")
			     + std::string(argv[0]) + std::string(" 1e-4 1000"));
  }
  ...
  for (auto & transpose : transposers) {
    ...
    if (transpose == fftwArr::Transposed::YES) {
      forward_pflags = FFTW_MPI_TRANSPOSED_OUT | FFTW_MEASURE;
      backward_pflags = FFTW_MPI_TRANSPOSED_IN | FFTW_MEASURE;
    } else {
    ...
    auto const time_elapsed{std::chrono::duration_cast<
	...
	std::cout << "\n\nSUCCESSFULLY CALCULATED THE GRADIENT OF FUNCTION "
		  ...
		  << "and average time elapsed " << global_latency << "us).\n\n"
		  << std::endl;
      } else {
	std::cout << "\n\nSUCCESSFULLY CALCULATED GRADIENT OF FUNCTION "
		  << "sin(x)*cos(y)*sin(2*z) USING FOURIER TRANSFORMS "
		  << "WITH FFTW_MPI_TRANSPOSE FLAGS "	  
		  << "(within an absolute error of " << tolerance
		  << "and average time elapsed " << global_latency << "us).\n\n"
		  << std::endl;
